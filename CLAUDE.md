...
| LLM coding runs (lab) | ✅ (local; not deployed) | Admin creates runs = {model + editable prompt + temperature}; each run codes the 404 tasks once, stored in `codings` with `run_id` + `source`=model slug + `raw_llm_response`. Model dropdown from cached OpenRouter list + free-text slug. `coding_runs` and `run_id` in exports. Legacy `coding_models` config path retained until this is deployed. |
| Rater QA drill-in | ✅ (local; not deployed) | Admin Coding tab: "Raters" overview (per `rater_pid`: n, median/min seconds, #under-20s, technique_count 1/2/3+ dist), most-suspicious-first, rows red when median <20s. Click a rater (or any `rater_pid` in All ratings) for a per-rater drill-in showing each rated text inline with scores + duration, sub-20s rows red. Read-only; pure aggregation in `app/coding_stats.php` (tested). |
